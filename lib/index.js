...
const toFlow = exports.toFlow = flowSchema => {
  const { leadingComments } = (0, _FlowTypeGenerator.toFlowType)(flowSchema);

  const flowType = _objectWithoutProperties((0, _FlowTypeGenerator.toFlowType)(flowSchema), ['leadingComments']);

  return _extends({}, t.exportNamedDeclaration(t.typeAlias(t.identifier((0, _FlowTypeGenerator.upperCamelCase)(flowSchema.$id)), null, flowType), []), {
    leadingComments
  });
};
