...
let FlowSchema = exports.FlowSchema = class FlowSchema {
  ...
  flowRef(flowRef) {
    if (_lodash2.default.isEmpty(flowRef)) {
      return this;
    }
  ...
  description(description) {
    if (_lodash2.default.isEmpty(description)) {
      return this;
    }
  ...
  format(format) {
    if (_lodash2.default.isEmpty(format)) {
      return this;
    }
  ...
  maxItems(maxItems) {
    if (_lodash2.default.isNil(maxItems)) {
      return this;
    }
  ...
  minItems(minItems) {
    if (_lodash2.default.isNil(minItems)) {
      return this;
    }
  ...
  uniqueItems(uniqueItems) {
    if (_lodash2.default.isNil(uniqueItems)) {
      return this;
    }
  ...
  minimum(minimum) {
    if (_lodash2.default.isNil(minimum)) {
      return this;
    }
  ...
  maximum(maximum) {
    if (_lodash2.default.isNil(maximum)) {
      return this;
    }
  ...
  exclusiveMinimum(exclusiveMinimum) {
    if (_lodash2.default.isNil(exclusiveMinimum)) {
      return this;
    }
  ...
  exclusiveMaximum(exclusiveMaximum) {
    if (_lodash2.default.isNil(exclusiveMaximum)) {
      return this;
    }
...
const convertSchema = exports.convertSchema = schema => {
  ...
  const f = flow().id(schema.id).flowRef(schema.$ref).enum(schema.enum).default(schema.default).description(schema.description).format(schema.format).definitions(_lodash2.default.mapValues(schema.definitions, (definition, id) => convertSchema(_lodash2.default.assign(_lodash2.default.omit(definition, '$ref'), { id }))));
  ...
  if (isArray(schema)) {
    return f.flowType('Array').union(_lodash2.default.map([].concat(schema.items || {}), convertSchema)).maxItems(schema.maxItems).minItems(schema.minItems).uniqueItems(schema.uniqueItems);
  }
  ...
  switch (_lodash2.default.toLower(String(schema.type))) {
    ...
    case 'number':
    case 'integer':
      return f.minimum(schema.minimum).maximum(schema.maximum).exclusiveMinimum(schema.exclusiveMinimum).exclusiveMaximum(schema.exclusiveMaximum).flowType('number');
    case 'boolean':
